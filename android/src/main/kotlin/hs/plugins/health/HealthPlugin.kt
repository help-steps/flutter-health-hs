...
class HealthPlugin(private var channel: MethodChannel? = null) : MethodCallHandler,
  ...
  override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
    if (requestCode == GOOGLE_FIT_PERMISSIONS_REQUEST_CODE) {
      if (resultCode == Activity.RESULT_OK) {
        Log.d("HS_HEALTH", "Access Granted!")
        mResult?.success(true)
      } else if (resultCode == Activity.RESULT_CANCELED) {
        Log.d("HS_HEALTH", "Access Denied!")
       // mResult?.success(false)
        mResult?.error("PERMISSION_DENIED", "User denied the permission", null)
      }
    }
    return false
  }
    ...
    try {
      ...
      Fitness.getHistoryClient(activity!!.applicationContext, googleSignInAccount)
        ...
        .addOnSuccessListener {
          Log.i("HS_HEALTH::SUCCESS", "DataSet added successfully!")
          result.success(true)
        }
        .addOnFailureListener { e ->
          Log.w("HS_HEALTH::ERROR", "There was an error adding the DataSet", e)
          result.success(false)
        }
    } catch (e3: Exception) {
      result.success(false)
  ...
  private fun writeWorkoutData(call: MethodCall, result: Result) {
    ...
    val activitySegmentDataSource = DataSource.Builder()
      ...
      .setStreamName("HS_HEALTH - Activity")
      .setType(DataSource.TYPE_RAW)
      .build()
    ...
    if (totalDistance != null) {
      ...
      val distanceDataSource = DataSource.Builder()
        ...
        .setStreamName("HS_HEALTH - Distance")
        .setType(DataSource.TYPE_RAW)
        .build()
    ...
    if (totalEnergyBurned != null) {
      ...
      val energyDataSource = DataSource.Builder()
        ...
        .setStreamName("HS_HEALTH - Calories")
        .setType(DataSource.TYPE_RAW)
        .build()
    ...
    try {
      ...
      Fitness.getSessionsClient(
        ...
        .addOnSuccessListener {
          Log.i("HS_HEALTH::SUCCESS", "Workout was successfully added!")
          result.success(true)

        }
        .addOnFailureListener { e ->
          Log.w("HS_HEALTH::ERROR", "There was a problem adding the workout: ", e)
          result.success(false)
        }
    } catch (e: Exception) {
      result.success(false)
    ...
    OnSuccessListener { response: DataReadResponse ->
      ...
      for (bucket in response.buckets) {
        ...
        if (dp != null) {
          ...
          Log.i("HS_HEALTH::SUCCESS", "returning $count steps for $startDate - $endDate")
          map[startTime] = count.asInt()
        } else {
          val startDay = Date(start)
          val endDay = Date(end)
          Log.i("HS_HEALTH::ERROR", "no steps for $startDay - $endDay")
        }
      }
