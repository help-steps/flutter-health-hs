...
class HealthFactory {
  ...
  Future<bool> requestAuthorization(
    ...
    if (permissions != null) {
      for (int i = 0; i < types.length; i++) {
        ...
        if ((type == HealthDataType.HIGH_HEART_RATE_EVENT ||
          ...
          throw ArgumentError(
              'Requesting WRITE permission on HIGH_HEART_RATE_EVENT / LOW_HEART_RATE_EVENT / IRREGULAR_HEART_RATE_EVENT / WALKING_HEART_RATE is not allowed.');
        }
      }
    }
    ...
    try {
      var isAuthorized = await _channel.invokeMethod(
